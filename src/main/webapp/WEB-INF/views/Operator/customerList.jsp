    ...
    <body>
        <div class="wrapper">
            ...
            <div class="container">
                ...
                <form class="form-inline" method="get" action="/Operator/customerList">
                ...
                <table class="table">
                    ...
                    <tbody>
                    <c:forEach items="${array}" var="customer">
                        <tr>
                            <th scope="row">${customer.id}</th>
                            <td>${customer.name}</td>
                            <td>${customer.lastName}</td>
                            <td>${customer.username}</td>
                            <td><form:form modelAttribute="customer" action="/Operator/appartmentList" method="get">
                                <form:hidden path="id" value="${customer.id}"/>
                                <form:hidden path="name" value="${customer.name}"/>
                                <form:hidden path="lastName" value="${customer.lastName}"/>
                                ...
                                <form:hidden path="phone" value="${customer.phone}"/>
                                <form:hidden path="password" value="${customer.password}"/>
                                <form:hidden path="address" value="${customer.address}"/>
                                <form:hidden path="birthday" value="${customer.birthday}"/>
                                <form:hidden path="idNumber" value="${customer.idNumber}"/>
                                <button type="submit" class="btn btn-primary mb-2">Prendre rendez-vous</button>
                            </form:form>
                            </td>
                        </tr>
                    </c:forEach>
